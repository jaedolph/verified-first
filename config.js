...
twitch.configuration.onChanged(function () {
  ...
  if (twitch.configuration.broadcaster) {
    try {
      console.log(
        "current config: " + twitch.configuration.broadcaster.content,
      );
...
async function displayConfigForm() {
  getRewards()
    ...
    .catch(function (error) {
      document.getElementById("config").innerHTML =
        "<p>ERROR: could not render configuration form.</p>";
      console.error(error);
    });
}
...
async function checkAuth() {
  ...
  if (!response.ok) {
    ...
    const errorMsg =
      "auth check failed: " + response.status + " " + responseText;
...
async function getRewards() {
  ...
  if (!response.ok) {
    return await response.text().then((text) => {
      const errorMessage =
        "could not get rewards: " + response.status + " " + text;
  ...
  switch (timeRange) {
    case "month":
      document
        ...
        .options.namedItem("month").selected = true;
      break;
    case "year":
      document.getElementById("time_range").options.namedItem("year").selected =
        true;
      break;
    default:
      document
        .getElementById("time_range")
        .options.namedItem("all_time").selected = true;
  }
...
async function createEventsub(rewardId) {
  const createEventsubUrl =
    extensionUri + "/eventsub/create?reward_id=" + rewardId;

  document.getElementById("eventsub").innerHTML =
    "Configuring channel point event listener...";
  ...
  if (!response.ok) {
    const responseText = await response.text();
    const errorMsg =
      "failed to create eventsub: " + response.status + " " + responseText;
    document.getElementById("eventsub").innerHTML =
