...
async function getFirsts(authorization, startTime, endTime) {
  ...
  if (startTime && !endTime) {
    firstsUrl =
      firstsUrl +
      "?" +
      new URLSearchParams({ start_time: startTime.toISOString() });
  }

  try {
    const response = await fetch(firstsUrl, {
      headers: {
    ...
    if (!response.ok) {
      firstsText.innerHTML =
        "Could not get leaderboard.<br>The extension may not be configured yet.";
